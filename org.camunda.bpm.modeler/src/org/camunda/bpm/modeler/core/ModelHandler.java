...
public class ModelHandler {
  ...
  public DataAssociation createDataAssociation(BaseElement source, BaseElement target) {
    ...
    if (source instanceof ItemAwareElement) {
      ...
      if (target instanceof Activity) {
        ...
        dataAssocation.setTargetRef(getOrCreateDataInput(activity));
   ...
   */
  public DataInput getOrCreateDataInput(Activity activity) {
    InputOutputSpecification ioSpecification = getOrCreateIoSpecification2(activity);
    List<DataInput> dataInputs = ioSpecification.getDataInputs();
    if (dataInputs.isEmpty()) {
      dataInputs.add(create(DataInput.class));
    }
    return dataInputs.get(0);
  }

  /**
   * In contrast to {@link #getOrCreateIOSpecification(Object)}, this method
   * creates the IO specification for the given Activity, not for the process.
   */
  private InputOutputSpecification getOrCreateIoSpecification2(Activity activity) {
    InputOutputSpecification ioSpecification = activity.getIoSpecification();
    if (ioSpecification == null) {
      ioSpecification = create(InputOutputSpecification.class);
  ...
  /**
   * General-purpose factory method that sets appropriate default values for new
   * objects.
   */
