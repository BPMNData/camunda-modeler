...
public class FeatureSupport {
    ...
    public static boolean isTargetSubProcess(ITargetContext context) {
        return BusinessObjectUtil.containsElementOfType(
                context.getTargetContainer(), SubProcess.class);
    }
    ...
    public static boolean isDiagram(PictogramElement shape) {
        Definitions definitions = BusinessObjectUtil.getFirstElementOfType(
                shape, Definitions.class);

        if (definitions != null) {
            return true;
        }

        return BusinessObjectUtil.containsChildElementOfType(shape,
                Definitions.class);
    }
    ...
    public static boolean isParticipant(PictogramElement element) {
        return BusinessObjectUtil.containsElementOfType(element,
                Participant.class);
    }

    public static Participant getTargetParticipant(ITargetContext context) {
        PictogramElement element = context.getTargetContainer();
        return BusinessObjectUtil.getFirstElementOfType(element,
                Participant.class);
    }

    public static boolean isLaneOnTop(Lane lane) {
        return lane.getChildLaneSet() == null
                || lane.getChildLaneSet().getLanes().isEmpty();
    }

    public static boolean isTargetLaneOnTop(ITargetContext context) {
        Lane lane = BusinessObjectUtil.getFirstElementOfType(
                context.getTargetContainer(), Lane.class);
        return lane.getChildLaneSet() == null
                || lane.getChildLaneSet().getLanes().isEmpty();
    }
    ...
    public static boolean isHorizontal(ContainerShape container) {
        ...
        if (parent instanceof PictogramElement) {
            ...
            if (BusinessObjectUtil.getFirstElementOfType(
                    (PictogramElement) parent, ChoreographyTask.class) != null)
                return false;
        }
        String v = Graphiti.getPeService().getPropertyValue(container,
    ...
    public static void setHorizontal(ContainerShape container,
        ...
        Graphiti.getPeService().setPropertyValue(container,
                IS_HORIZONTAL_PROPERTY, Boolean.toString(isHorizontal));
        BPMNShape bs = BusinessObjectUtil.getFirstElementOfType(container,
                BPMNShape.class);
        if (bs != null)
    ...
    public static List<PictogramElement> getContainerChildren(
        ...
        for (PictogramElement pe : container.getChildren()) {
            String value = Graphiti.getPeService().getPropertyValue(pe,
                    ACTIVITY_DECORATOR);
    ...
    public static List<PictogramElement> getContainerDecorators(
            ContainerShape container) {
        List<PictogramElement> list = new ArrayList<PictogramElement>();
        for (PictogramElement pe : container.getChildren()) {
            String value = Graphiti.getPeService().getPropertyValue(pe,
    ...
    public static void setContainerChildrenVisible(ContainerShape container,
            boolean visible) {
    ...
    public static void recursiveResizeChildren(ContainerShape container,
            ResizeDiff diff, IFeatureProvider featureProvider) {

        Sector direction = diff.getResizeDirection();
        Point delta = diff.getResizeDelta();

        if (direction.isTop() || direction.isBottom()) {
            recursiveResizeChildrenVertically(container, direction.isTop(),
                    delta.getY(), featureProvider);
        }

        if (direction.isLeft() || direction.isRight()) {
            recursiveResizeChildrenHorizontally(container, direction.isLeft(),
                    delta.getX(), featureProvider);
        }
    }

    public static void restoreFlowElementPositions(ContainerShape container,
            Map<Shape, IRectangle> preMoveBounds,
    ...
    private static void recursiveResizeChildrenHorizontally(
            ContainerShape container, boolean left, int dx,
            IFeatureProvider featureProvider) {
        ...
        for (Shape childLane : childLanes) {
            ...
            recursiveResizeChildrenHorizontally((ContainerShape) childLane,
                    left, dx, featureProvider);
    ...
    private static void recursiveResizeChildrenVertically(
            ContainerShape container, boolean top, int dy,
            IFeatureProvider featureProvider) {
        ...
        recursiveResizeChildrenVertically((ContainerShape) resizeCandidate,
                top, dy, featureProvider);
        ...
        for (Shape childLane : childLanes) {
            ...
            Graphiti.getGaService().setLocation(childLaneGA,
                    childLaneGA.getX(), y - m);
    ...
    public static Map<Shape, IRectangle> buildFlowElementPositionMap(
            ContainerShape container) {
    ...
    public static void resizeLaneSet(ContainerShape container) {
        ...
        if (idx > 0) {
            ...
            if (siblingGA.getY() + siblingGA.getHeight() != containerGa.getY() + 1) {
                topResize = containerGa.getY()
                        - (siblingGA.getY() + siblingGA.getHeight());
            }
        }
        ...
        for (Shape shape : childLanes) {
            ...
            Graphiti.getGaService().setLocation(shapeGA,
                    GraphicsUtil.PARTICIPANT_LABEL_OFFSET, y - m);
        ...
        if (topResize != 0) {
            Graphiti.getGaService().setLocation(parentGa, parentGa.getX(),
                    parentGa.getY() + topResize);
        }
    ...
    public static ContainerShape getRootContainer(ContainerShape container) {
        ...
        EObject bo = BusinessObjectUtil.getFirstElementOfType(parent,
                BaseElement.class);
    ...
    private static IRectangle resize(ContainerShape container) {
        ...
        EObject elem = BusinessObjectUtil.getFirstElementOfType(container,
                BaseElement.class);
        ...
        for (Shape s : children) {
            Object bo = BusinessObjectUtil.getFirstElementOfType(s,
                    BaseElement.class);
            if (bo != null && (bo instanceof Lane || bo instanceof Participant)
                    && !bo.equals(elem)) {
        ...
        if (horz) {
            if (height == 0) {
                return rectangle(ga.getX(), ga.getY(), ga.getWidth(),
                        ga.getHeight());
            } else {
    ...
    private static IRectangle resizeRecursively(ContainerShape root,
            ContainerShape resizedShape) {

        BaseElement elem = BusinessObjectUtil.getFirstElementOfType(root,
                BaseElement.class);
        IGaService service = Graphiti.getGaService();
        ...
        for (Shape s : root.getChildren()) {
            ...
            if (checkForResize(elem, s, bo)) {
                foundContainers += 1;
                IRectangle bounds = resizeRecursively((ContainerShape) s,
                        resizedShape);
                if (bounds != null) {
     ...
     */
    private static boolean checkForResize(BaseElement currentBo, Shape s,
            Object bo) {
    ...
    private static void postResizeFixLenghts(ContainerShape root) {
        ...
        for (Shape s : root.getChildren()) {
            Object o = BusinessObjectUtil.getFirstElementOfType(s,
                    BaseElement.class);
    ...
    /**
     * Returns the first child shape of the container that fulfills the
     * property.
     * 
     * @param context
     * @param propertyKey
     ...
     * @return
     */
    public static Shape getChildShapeFulfillingProperty(
            IPictogramElementContext context, String propertyKey,
        ...
        if (pe instanceof ContainerShape) {
            ...
            for (Shape shape : cs.getChildren()) {
                ...
                if (propertyValue.equals(peService.getPropertyValue(shape,
                        propertyKey))) {
                    return shape;
                }
            }
        } else if (pe instanceof Shape) {
            if (propertyValue.equals(peService
                    .getPropertyValue(pe, propertyKey))) {
                return (Shape) pe;
            }
        }
        return null;
     ...
     * @return
     */
    public static ConnectionDecorator getConnectionDecoratorFulfillingProperty(
            IPictogramElementContext context, String propertyKey,
            String propertyValue) {
        PictogramElement pe = context.getPictogramElement();
        if (pe instanceof Connection) {
            ...
            for (ConnectionDecorator decorator : connection
                    .getConnectionDecorators()) {
                IPeService peService = Graphiti.getPeService();
                if (propertyValue.equals(peService.getPropertyValue(decorator,
                        propertyKey))) {
                    return decorator;
                }
            }
    ...
    public static String getBusinessValue(IPictogramElementContext context) {
        Object o = BusinessObjectUtil.getFirstElementOfType(
                context.getPictogramElement(), BaseElement.class);
    ...
    public static Participant getTargetParticipant(ITargetContext context,
        ...
        Object bo = BusinessObjectUtil.getFirstElementOfType(
                context.getTargetContainer(), BaseElement.class);
    ...
    public static Shape getShape(ContainerShape container, String property,
    ...
    public static <T extends EObject> T getChildElementOfType(
            PictogramElement container, String property, String expectedValue,
            Class<T> clazz) {
        IPeService peService = Graphiti.getPeService();
     ...
     * assigned businessObjectClazz, i.e. the list contains {@link Shape}s which
     * meet the following constraint:<br>
     * <code>
     ...
     * @return
     */
    public static List<Shape> getChildrenLinkedToType(ContainerShape root,
            Class<?> businessObjectClazz) {
        List<Shape> result = new ArrayList<Shape>();
        for (Shape currentShape : root.getChildren()) {
            if (BusinessObjectUtil.containsChildElementOfType(currentShape,
                    businessObjectClazz)) {
    ...
    public static ContainerShape getLaneBefore(ContainerShape container) {
        ...
        for (PictogramElement picElem : getChildrenLinkedToType(
                parentContainerShape, Lane.class)) {
            if (picElem instanceof ContainerShape && !picElem.equals(container)) {
                ContainerShape currentContainerShape = (ContainerShape) picElem;
                GraphicsAlgorithm currentGA = currentContainerShape
                        .getGraphicsAlgorithm();
                if (isHorizontal) {
                    if (currentGA.getY() < y) {
                        if (result != null) {
                            GraphicsAlgorithm resultGA = result
                                    .getGraphicsAlgorithm();
    ...
    /**
     * Move all children in the given container by delta to compensate the
     * container movement
     * 
     * @param container
     * @param delta
     ...
     */
    public static void moveChildren(ContainerShape container, Point delta,
            IFeatureProvider featureProvider) {
        moveShapes(new ArrayList<Shape>(container.getChildren()), delta,
                featureProvider);
    }

    public static void moveShapes(ArrayList<Shape> shapes, Point delta,
            IFeatureProvider featureProvider) {
    ...
    private static void moveShapeNoLayout(Shape shape, Point delta,
            IFeatureProvider featureProvider) {

        ILocation location = LayoutUtil.getRelativeBounds(shape);

        MoveShapeContext moveContext = new MoveShapeContext(shape);
        moveContext.setLocation(location.getX() + delta.getX(), location.getY()
                + delta.getY());
        moveContext.setSourceContainer(shape.getContainer());
        moveContext.setTargetContainer(shape.getContainer());

        ContextUtil.set(moveContext,
                DefaultMoveBPMNShapeFeature.SKIP_REPAIR_CONNECTIONS_AFTER_MOVE);
        ContextUtil.set(moveContext,
                DefaultMoveBPMNShapeFeature.SKIP_MOVE_BENDPOINTS);

        IMoveShapeFeature moveShapeFeature = featureProvider
                .getMoveShapeFeature(moveContext);

        executeFeature(moveShapeFeature, moveContext);
    }

    public static void triggerAddFeature(ContainerShape container,
            BaseElement element, IFeatureProvider featureProvider) {
    ...
    public static boolean isBpmn20Shape(EObject element) {
        if (element instanceof ContainerShape) {
            BaseElement be = BusinessObjectUtil
                    .getFirstBaseElement((Shape) element);
            return be != null;
        } else {
            return false;
        }
     ...
     *             if the feature is null or it does not execute in the given
     *             context.
     */
    public static void executeFeature(IFeature feature, IContext context) {
        ...
        if (feature == null) {
            throw new IllegalArgumentException(String.format(
                    "No executable feature for context %s", context));
        }
        ...
        if (feature.canExecute(context)) {
            ...
            throw new IllegalArgumentException(String.format(
                    "Failed to execute move feature %s", feature));
        }
    }
    ...
    public static List<Shape> getChildLanes(ContainerShape rootShape) {
        ...
        List<Shape> childLanes = FeatureSupport.getChildrenLinkedToType(
                rootShape, Lane.class);
        if (childLanes.isEmpty()) {
    ...
    public static void eachLaneExecute(ContainerShape container,
            LaneSetOperation operation) {
    ...
    private static void recursiveEachLaneExecute(ContainerShape container,
            LaneSetOperation operation) {
