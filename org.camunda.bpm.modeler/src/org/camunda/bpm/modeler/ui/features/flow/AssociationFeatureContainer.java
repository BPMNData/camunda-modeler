...
public class AssociationFeatureContainer extends BaseElementConnectionFeatureContainer {
    ...
    return new AbstractAddFlowFeature<Association>(fp) {
      ...
      public PictogramElement add(IAddContext context) {
        ...
        if (sourceAnchor == null && source instanceof FreeFormConnection) {
          Shape connectionPointShape = AnchorUtil.createConnectionPoint(getFeatureProvider(), (FreeFormConnection) source, Graphiti.getPeLayoutService()
              .getConnectionMidpoint((FreeFormConnection) source, 0.5));
        ...
        if (targetAnchor == null && target instanceof FreeFormConnection) {
          Shape connectionPointShape = AnchorUtil.createConnectionPoint(getFeatureProvider(), (FreeFormConnection) target, Graphiti.getPeLayoutService()
              .getConnectionMidpoint((FreeFormConnection) target, 0.5));
          targetAnchor = AnchorUtil.getConnectionPointAnchor(connectionPointShape);
          anchorChanged = true;
        }

        // this is silly! why are there no setters for sourceAnchor and
        // targetAnchor in AddConnectionContext???
        if (anchorChanged) {
  ...
  public class CreateAssociationFeature extends AbstractCreateFlowFeature<Association, BaseElement, BaseElement> {
    ...
    protected void postCreateHook(ICreateConnectionContext context, Association bo) {
      ...
      if (bo.getSourceRef() != null) {
        if (bo.getSourceRef() instanceof MessageFlow) {
          messageFlow = (MessageFlow) bo.getSourceRef();
        } else if (bo.getSourceRef() instanceof Message) {
          message = (Message) bo.getSourceRef();
        }
      }
    
    if (bo.getTargetRef() != null) {
      if (bo.getTargetRef() instanceof MessageFlow) {
        messageFlow = (MessageFlow) bo.getTargetRef();
      } else if (bo.getTargetRef() instanceof Message) {
        message = (Message) bo.getTargetRef();
      }
    }
  ...
  public static class ReconnectAssociationFeature extends AbstractReconnectFlowFeature {
    ...
    public void preReconnect(IReconnectionContext context) {
      PictogramElement targetPictogramElement = context.getTargetPictogramElement();
      if (targetPictogramElement instanceof FreeFormConnection) {
        Shape connectionPointShape = AnchorUtil.createConnectionPoint(getFeatureProvider(), (FreeFormConnection) targetPictogramElement,
